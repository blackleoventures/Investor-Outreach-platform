...
export async function POST(
  ...
  try {
    ...
    if (!recipientIds || !Array.isArray(recipientIds) || recipientIds.length === 0) {
      return NextResponse.json(
        { success: false, error: 'recipientIds array is required and must not be empty' },
        { status: 400 }
      );
    }

    if (!followupType || !['not_opened', 'opened_not_replied'].includes(followupType)) {
      return NextResponse.json(
        { success: false, error: 'followupType must be either "not_opened" or "opened_not_replied"' },
        { status: 400 }
      );
    }
    ...
    if (!generated.success) {
      return NextResponse.json(
        { success: false, error: 'Failed to generate email with AI', details: generated.error },
        { status: 500 }
      );
    }
