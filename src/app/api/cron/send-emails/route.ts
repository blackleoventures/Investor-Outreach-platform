...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    console.log('[Cron: Send Emails] Found recipients:', recipientsSnapshot.size);
    ...
    if (alreadyProcessed.length > 0) {
      console.log('[Cron: Send Emails] Detected race condition, some recipients already processed');
    }
    ...
    recipientsSnapshot.forEach((doc) => {
      ...
      if (data.status !== "pending") {
        console.log('[Cron: Send Emails] Skipping non-pending recipient:', doc.id);
        return;
      }
    ...
    console.log('[Cron: Send Emails] Campaigns to process:', Object.keys(campaignGroups).length);
    ...
    for (const [campaignId, recipients] of Object.entries(campaignGroups)) {
      ...
      console.log('[Cron: Send Emails] Recipients in campaign:', recipients.length);
      ...
      try {
        ...
        if (!campaignData) {
          console.error('[Cron: Send Emails] Campaign has no data:', campaignId);
          continue;
        }

        // FIXED: Check if campaign is actually active
        if (campaignData.status !== "active") {
          console.log('[Cron: Send Emails] Campaign not active:', campaignId, 'Status:', campaignData.status);
          continue;
        }
        ...
        if (!clientDoc.exists) {
          console.error('[Cron: Send Emails] Client not found:', campaignData.clientId);
          continue;
        }
        ...
        if (!clientData) {
          console.error('[Cron: Send Emails] Client has no data:', campaignData.clientId);
          continue;
        }
        ...
        console.log('[Cron: Send Emails] SMTP Config loaded for campaign:', campaignId);

        if (!smtpConfig || !smtpConfig.smtpHost || !smtpConfig.smtpUsername) {
          console.error('[Cron: Send Emails] Invalid SMTP configuration for campaign:', campaignId);
        ...
        try {
          ...
          console.log('[Cron: Send Emails] SMTP password decrypted successfully');
        } catch (error: any) {
          console.error('[Cron: Send Emails] Failed to decrypt SMTP password:', error);
        ...
        try {
          ...
          console.log('[Cron: Send Emails] SMTP connection verified successfully');
        } catch (error: any) {
          console.error('[Cron: Send Emails] SMTP verification failed');
          logError('SMTP Verification', error, { campaignId, smtpHost: smtpConfig.smtpHost });
          ...
          try {
            ...
            const transactionResult = await adminDb.runTransaction(async (transaction) => {
              ...
              if (!freshRecipientDoc.exists) {
                console.log('[Cron: Send Emails] Recipient deleted during processing:', recipient.id);
              ...
              if (freshData!.status !== "pending") {
                console.log('[Cron: Send Emails] Recipient status changed during processing:', recipient.id, 'Status:', freshData!.status);
              ...
              if (freshData!.emailHistory && freshData!.emailHistory.length > 0) {
                ...
                if (timeSinceLastSend < 300000) {
                  console.log('[Cron: Send Emails] Skipping duplicate - email sent recently:', freshData!.originalContact?.email);
                  console.log('[Cron: Send Emails] Last sent:', lastEmail.sentAt, 'Time since:', timeSinceLastSend, 'ms');
            ...
            console.log('[Cron: Send Emails] Sending to recipient:', recipientEmail);
            ...
            if (info.accepted && info.accepted.length > 0) {
              ...
              campaignSent++;
              totalSent++;
              console.log('[Cron: Send Emails] Email sent successfully:', recipientEmail);
            } else {
            ...
            console.error('[Cron: Send Emails] Send failed for recipient:', recipientEmail);
            ...
            if (retryAttempt < 3 && canRetryError(errorCategory)) {
              ...
              console.log('[Cron: Send Emails] Retry scheduled for:', recipientEmail);
              console.log('[Cron: Send Emails] Retry time:', nextRetryTime.toISOString());
              console.log('[Cron: Send Emails] Retry attempt:', retryAttempt);
            } else {
              console.log('[Cron: Send Emails] No retry scheduled:', 
                retryAttempt >= 3 ? 'Max retries reached' : 'Error not retryable'
              );
            }
          }
        }
        ...
        console.log('[Cron: Send Emails] Skipped (duplicates):', campaignSkipped);

        transporter.close();

      } catch (error: any) {
        console.error('[Cron: Send Emails] Campaign processing error:', campaignId);
    ...
    console.log('[Cron: Send Emails] Total skipped (duplicates):', totalSkipped);
    console.log('[Cron: Send Emails] Error breakdown:', errorBreakdown);
    console.log('[Cron: Send Emails] Campaigns processed:', Object.keys(campaignGroups).length);
    console.log('[Cron: Send Emails] Duration:', duration + 'ms');
    ...
    return NextResponse.json({
      ...
      summary: {
        ...
        pending: recipientsSnapshot.size - totalSent - totalFailed - totalSkipped,
